/* ****************************************************************************
*  MvcControlToolkit.Bindings-2.5.0.js
...
(function () {
    ...
    mvcct.utils.evalAndDestroyTemplateScripts = function (x) {
...
function MvcControlsToolkit_InitializeCreatedNodes(elements, unobtrusiveValidation, noJavaScript, applyValidation, delayedBatch) {
    ...
    toApply = function () {
        for (var i = 0; i < elements.length; i++) {
            ...
            if (!noJavaScript) mvcct.utils.evalAndDestroyTemplateScripts(elements[i]);
        ...
        ko.utils.arrayForEach(eventsToCatch, function (eventName) {
            ...
            ko.utils.registerEventHandler(element, eventName, function () {
                runEventHandler(function () {
                    ...
                    if (elementType != "") {
                        elementValue = window["MvcControlsToolkit_" + elementType + "_Get"](element, valueType);
                    }
                    else {
        ...
        if (valueHasChanged) {
            ...
            if (elementType != "") {
                applyValueAction = function () {
                    window["MvcControlsToolkit_" + elementType + "_Set"](element, newValue, formatString, valueType);
...
(function () {
    ...
    ko.nativeTemplateEngine.prototype['renderTemplateSource'] = function (templateSource, bindingContext, options) {
        ...
        return jQuery.parseHTML ? jQuery.parseHTML(templateText, null, true) : ko.utils.parseHtmlFragment(templateText);
    };
...
ko.underscoreTemplateEngineExt.prototype = ko.utils.extend(new ko.templateEngine(), {
    ...
    renderTemplateSource: function (templateSource, bindingContext, options) {
        ...
        return jQuery.parseHTML ? jQuery.parseHTML(renderedMarkup, null, true) : ko.utils.parseHtmlFragment(renderedMarkup);
    },
...
(function () {
    ...
    ko.bindingHandlers['template']['update'] = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        if (!bindingContext.recall && oldBeforeRemove && !isForeach) {
            var allchilds = [];
            for (var x = ko.virtualElements.firstChild(element) ; x; x = ko.virtualElements.nextSibling(x)) allchilds.push(x);
        ...
        if (isDetail) {
            ...
            for (var x = ko.virtualElements.firstChild(element) ; x; x = ko.virtualElements.nextSibling(x)) {
                ...
                var curr = $(x);
                if (curr.data('isCached')) toClear.push(curr);
                if ($.validator && $.validator.testEntity) handleErrors(x);
            }
            ...
            for (var i = 0; i < toClear.length; i++) {
                if (newFather) toClear[i].appendTo(newFather);
                else toClear[i].detach();
            }
        ...
        var defaultOptions =
    {
        ...
        if (mvcct.ko && mvcct.ko.observableTracking) {
            bindingValue.beforeBind = function (x, y) {
                for (var i = 0; i < x.length; i++) {
                    var jel = $(x[i]);
                    if (jel.data('isCached') || false) return true;
                }
                mvcct.utils.currentModel = y;
